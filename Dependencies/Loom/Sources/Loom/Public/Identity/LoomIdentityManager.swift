...
public final class LoomIdentityManager {
    ...
    private func savePrivateKey(_ key: P256.Signing.PrivateKey) throws {
        ...
        } catch LoomIdentityError.keychainWriteFailed(let status)
            where synchronizable && Self.isSynchronizableWriteUnsupported(status) {
            // Synchronizable items require an application-identifier
            // entitlement (and iCloud Keychain). Apps without provisioning,
            // and macOS 13 login keychains, reject them, so fall back to a
            // device-local item. Reads query kSecAttrSynchronizableAny and
            // find either kind.
            LoomLogger.trust(
                "Synchronizable identity write unavailable (status: \(status)); storing device-local identity key"
            )
            try savePrivateKey(key, synchronizable: false)
        }
    }

    /// Whether a keychain write failure indicates synchronizable items are
    /// unsupported for this process (missing entitlement or no iCloud Keychain).
    private nonisolated static func isSynchronizableWriteUnsupported(_ status: OSStatus) -> Bool {
        status == errSecMissingEntitlement || status == errSecParam
    }

    private func savePrivateKey(_ key: P256.Signing.PrivateKey, synchronizable: Bool) throws {
